...
## Example Use Case
**Assembly line scenario**: For a computer mother board assembly line (Figure 2), characterized by Assembly Line Steps (ALS), at Assembly Line Step 3 (ALS_03) the solder paste is applied on the empty board, and then a regular test is performed to check that the paste droplets are not too big or too small and that they are at the center or each pad. Boards than pass this **already in place** test, move to next assembly line step and eventually the electronic circuits are placed on the board and then connected to the board pads by melting the solder droplets in an oven. At the end of ALS 5 (ALS_05), a final electronic functional check is done before the board is shipped to client OEM PC manufacturer. Some of the failures discovered at ALS_05 are due to defective solder paste application performed in ALS_03, even if the board **passed the regular ALS_03 test**. We can use **Machine Learning (ML)** to predict **solder related** failures right after the solder paste is applied in ALS_03, but **before the expensive electronic components** like microprocessors and other integrated circuits are added to the board at ALS_05. Rewashing a board with a functional defect that **passed the regular QA test** at ALS_01 may cost cents, while scrapping a defective board at ALS_05 after the processor and all other components are applied may cost hundreds of dollars. 
